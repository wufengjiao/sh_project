...
class CreateUserTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::connection($connection)->create(config('user'), function (Blueprint $table) {//用户表
        ...
        Schema::connection($connection)->create(config('power'), function (Blueprint $table) {//权限表
            $table->engine = 'InnoDB';//指定表类型
        ...
        schema::connection($connection)->create(config('menu'),function (Blueprint $table){//菜单表
            $table->engine = 'InnoDB';//指定表类型
        ...
        schema::connection($connection)->create(config('index'),function (Blueprint $table){//菜单表
            $table->engine = 'InnoDB';//指定表类型
    ...
    public function down()
    {
        ...
        Schema::connection($connection)->dropIfExists(config('user'));
        Schema::connection($connection)->dropIfExists(config('power'));
        Schema::connection($connection)->dropIfExists(config('menu'));
        Schema::connection($connection)->dropIfExists(config('index'));
    }
}
